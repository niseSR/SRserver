...
public class ArcinfoController {
	...
	public ModelAndView submit_StakeholderInfo(HttpServletRequest req, HttpServletResponse res) throws Exception {
		...
		// 최종 insertrk 완료되면 user의 Stakeholder counter를 1 올린다.(추후에 삭제시 1 빼는 것 로직
		// 추가 예정)
	...
	public ModelAndView submit_PlatformInfo(HttpServletRequest req, HttpServletResponse res) throws Exception {
		...
		String platformID = "cpe:2.3:" + platform.getPlatformPart() + ":" + platform.getPlatformVendor() + ":"
				+ platform.getPlatformProduct() + ":" + platform.getPlatformVersion() + ":"
				+ platform.getPlatformUpdate() + ":" + platform.getPlatformEdition() + ":"
				+ platform.getPlatformLanguage() + ":" + platform.getPlatformSoftwareEdition() + ":"
				+ platform.getPlatformTargetSoftware() + ":" + platform.getPlatformTargetHardware() + ":"
				+ platform.getPlatformOther();
	...
	// 맨처음 페이지 로드할 때 session에 입력된 사용자 정보를 바탕으로 Stakeholder의 정보를 가져오고 이를 체크박스화 하는
	// 로직
	...
	// 여기서 Domain Asset 정보와 Platform 정보, 연관 Stakeholder 정보는 데이터베이스 정규화로 인해서 분리해서
	// 저장됨
	...
	public ModelAndView submit_DomainAssetInfo(HttpServletRequest req, HttpServletResponse res) throws Exception {
		...
		// user의 Domain Asset의 count number 가져오기(사용자 Usability를 위해 자동으로 ID를 부여하기
		// 위한 작업임)
		...
		// 최종 insert가 완료되면 user의 Stakeholder counter를 1 올린다.(추후에 삭제시 1 빼는 것 로직
		// 추가 예정)
	...
	public ModelAndView submit_DomainAssetPLSHInfo(HttpServletRequest req, HttpServletResponse res) throws Exception {
		...
		// 데이터를 hash map에 집어넣고 반복적으로 table에 입력(Domain Asset 과 관련 Stakeholder 정보
		// 종합)
	...
	public ModelAndView select_ImplCountermeasureType(HttpServletRequest req, HttpServletResponse res)
			throws Exception {
		RequestParameter rp = Utils.extractRequestParameters(req);
		ModelAndView mnv = new ModelAndView("/common/json_result");
		Map<String, Object> map = new HashMap<String, Object>();
